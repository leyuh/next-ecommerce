...
const Slider = () => {
    ...
    return <div className="h-[calc(100vh-80px)] overflow-hidden">
        ...
        <div className="absolute m-auto left-1/2 bottom-8 flex gap-4">
            {slides.map((slide, i) => <div key={slide.id} onClick={() => setCurrent(i)} className={`w-3 h-3 rounded-full ring-1 ring-gray-600 cursor-pointer flex items-center justify-center ${current === i ? "scale-150" : ""}`}>
                {current === i && <div className="w-[6px] h-[6px] bg-gray-600 rounded-full"></div>}
            </div>)}
        </div>
    </div>
}
